# Usefully
Usefull links for webdev, server management, and my lessons learned.

## Server management

### OS

### Database
- [Docs MariaDB](https://mariadb.com/kb/en/documentation/)
- [mariadb installation guide](https://geekflare.com/mariadb-installation-guide/)
- [How to install mariadb on debian 10](https://www.digitalocean.com/community/tutorials/how-to-install-mariadb-on-debian-10)
- [How to uninstall mariadb completely](https://mariadb.com/kb/en/uninstall-or-delete-mariadb-completely-for-re-installation/)

### Misc
- [Linux search files from the terminal - locate](https://www.howtoforge.com/tutorial/linux-search-files-from-the-terminal/)
- [Remove files with specific match](https://www.linuxshelltips.com/remove-files-with-specific-extension-in-linux/)
- [List enabled virtual hosts](https://www.tecmint.com/list-enabled-virtual-hosts-in-apache-web-server/)
- [VirtualHost Macros](https://httpd.apache.org/docs/2.4/mod/mod_macro.html)
- [VIM - find and replace](https://linuxize.com/post/vim-find-replace/)

## Web development

### Back-end

- [Make sure to update used PHP version in Apache](https://serverfault.com/a/815479/154604)
- [Make PHP show errors instead of blank 500 page](https://stackoverflow.com/questions/2687730/how-can-i-make-php-display-the-error-instead-of-giving-me-500-internal-server-er)
- [New in PHP 8](https://stitcher.io/blog/new-in-php-8)

### Front-end

## IDE and Software
- [MySQL Workbench - fingerprint has changed fix](https://stackoverflow.com/a/57922063/1110760)
